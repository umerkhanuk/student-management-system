...
let stuRegistration: any = await inquirer.prompt([
...
if (
  ...
  console.log(
    chalk.yellowBright(
      "Artificial Intelligence = 30000 PKR Per Semester \nWeb3 and Metaverse = 30000 PKR Per Semester \nCloud-Native Computing = 30000 PKR Per Semester\n"
    )
  );
  let confirmation: any = await inquirer.prompt({
    message: "Do you want to Login?",
    name: "confirm",
    type: "confirm",
  });
  if (confirmation.confirm === true) {
    let stuSignup: any = await inquirer.prompt([
      { message: "Student Name: ", name: "name", type: "input" },
      { message: "Password: ", name: "pass", type: "password" },
    ]);
    if (
      stuSignup.name === stuRegistration.name &&
      ...
      if (fees.fees_submit === true) {
        ...
        if (account.accountDetails === "Bank Transfer") {
          ...
          if (bankTransfer.confirm === true && bankTransfer.amount === 30000) {
            console.log(
              chalk.greenBright("Fees Has Been Submit Successfully!!")
            );
            idCard = true;
          } else {
